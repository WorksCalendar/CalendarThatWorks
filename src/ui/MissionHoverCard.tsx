...
import type { MissionAssignments, AssignedResource, MissionRequirements } from '../types/mission';
...
export interface MissionComplianceItem {
  id: string;
  label: string;
...
export type MissionSlotKind = 'pilot' | 'medical' | 'aircraft';
...
function pilotById(id: string, employees: MissionCrewMember[]): MissionCrewMember | undefined {
  return employees.find(e => e.id === id && e.role === 'pilot');
}

function medicalById(id: string, employees: MissionCrewMember[]): MissionCrewMember | undefined {
  return employees.find(e => e.id === id && (e.role === 'rn' || e.role === 'rt' || e.role === 'medic'));
}

function aircraftById(id: string, fleet: MissionAircraft[]): MissionAircraft | undefined {
  return fleet.find(a => a.id === id);
}

export function meetsAircraftReqs(ac: MissionAircraft, mission: MissionRequest): boolean {
...
export default function MissionHoverCard({
  ...
  const openSlot = useCallback((kind: MissionSlotKind, index: number, rect: DOMRect) => {
    setError(null);
