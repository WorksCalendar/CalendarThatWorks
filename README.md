...
Clicking **✕ Clear Status** in the dropdown resets both the status and any assigned coverage in one step.

### Where state lives

Coverage is stored in each event's `meta` field so it travels with normal `onEventSave` callbacks and survives engine undo/redo:
...
See [`examples/08-ShiftCoverageTracking.jsx`](./examples/08-ShiftCoverageTracking.jsx) for a full runnable example.
...
| Shift coverage tracking (PTO / on-call) | [`examples/08-ShiftCoverageTracking.jsx`](./examples/08-ShiftCoverageTracking.jsx) |
